...
def use_items(self, cible, item) : 
	...
	for group in ls : 
		if int(group[0]) == number : 
			if int(group[1]) == 0 : # La personne ne possède pas l'item qu'il veut utiliser.
	...
	if item == 1 : 
		...
		if int(db[sablier_dans_db]) == 0 : 
			...
			my_list = [daily_dans_db, beg_dans_db, hebdo_dans_db, steal_dans_db]
...
def get_items_of_user(self, cible) : 
	...
	for group_item in range(len(ls)) : 
		item_in_shop = self.shop[(int(ls[group_item][0])) - 1] # 0 : Name		1 : Comment		2 : Prix		3 : Number
		name = item_in_shop[0]
...
def create_user(self, UserToCreate) : 
	...
	parcour = [0, 1, 2, 3, 5, 9, 10, 11, 12, 13] # Daily, Hebdo, Gold, Daily, Steal, [...], Argent rapportée en exploit. pétrol., Durée d'inactivité...
	users = db.keys()
...
def extract_data_encoded_NT1(self, cible): # NT1 Nolann's Technic 1 (spécifique)
	items_dans_db_for_author = self.prefixes[6] + f"{cible}"
	try : 
...
class commands : 
	...
	async def buy(self, notation, args) : 
		...
		try : 
			...
			embed = discord.Embed(title=f"Merci de faire ```{notation}```", description="Merci d'entrer un nombre et non du texte.", color=white)
			await self.channel.send(embed=embed)
			return
		except IndexError : 
			embed = discord.Embed(title=f"Merci de faire ```{notation}```", description="Merci d'entrer un nombre indiquant l'objet que vous souhaitez acheter.", color=white)
			await self.channel.send(embed=embed)
			return
		...
		try : 
			count = int(args[1])
			if count < 0 : 
				embed = discord.Embed(title=f"Vous n'essayez quand même pas de me vendre ces objets ?!", description="Non merci, je refuse. Achetez plutout en positif.", color=white)
				await self.channel.send(embed=embed)
				return
		except ValueError : 
			embed = discord.Embed(title=f"Merci de faire ```{notation}```", description="Merci d'entrer un nombre et non du texte.", color=white)
		...
		if (item < 0) or (item > len(self.shop)) : 
			embed = discord.Embed(title=f"Merci de faire ```{notation}```", description="L'objet demandé n'existe pas !", color=white)
			await self.channel.send(embed=embed)
			return

		item_in_shop = self.shop[item] # 0 : Name		1 : Comment		2 : Prix		3 : Number
		name = item_in_shop[0]
		...
		if not ((gold_of_cible - valeur) >= 0) : # Trop pauvre !
			embed = discord.Embed(title="Vous ne possédez pas assez d'argent pour acheter cet objet !", description=f"Vous possédez {gold_of_cible} :dollar: et votre achat vous coûterai {valeur} :dollar: !", color=white)
			await self.channel.send(embed=embed)
			return
		...
		db[gold_dans_db_for_cible] = str(gold_of_cible - valeur) # On retire le prix de l'objet.
		...
		embed = discord.Embed(title="", description=f"Vous avez acheté {count} {name} pour {valeur} :dollar: !", color=white)
		await self.channel.send(embed=embed)


	async def sell(self, notation, args) : 
		embed = discord.Embed(title="La commande sell n'est pas encore disponible.", description="Contactez Polaris#4776 si vous voulez qu'elle soit ajoutée.", color=white)
		await self.channel.send(embed=embed)
	...
	async def bag(self, notation, args) : 
		...
		embed = discord.Embed(title=f"Inventaire de {cible}", description=f"{formated}", color=white)
		await self.channel.send(embed=embed)


	async def use (self, notation, args) : 
		if len(args) == 0 : 
			embed = discord.Embed(title=f"Merci de faire ```{notation}```", description="", color=white)
			await self.channel.send(embed=embed)
			return
	...
	async def profile (self, notation, args) : 
		...
		cube = "▬" * 18
		embed = discord.Embed(title=f"Profil de {cible}", description=f"{cube}\n{gold}\n{level}\n{inactivity}\n{cube}\n{equip_and_power}\n{cube}\n{objects}", color=white)
		await self.channel.send(embed=embed)
	...
	async def gold (self, notation, args) : 
		...
		if (cible == "None") or (cible == str(self.author)) : 
			...
			embed = discord.Embed(title=f"{self.author}", description="Tu as {} :dollar:.".format(db[gold_dans_db]), color=white)
			await self.channel.send(embed=embed)

		else : 
			gold_dans_db = self.prefixes[2] + cible

			embed = discord.Embed(title="", description="Cette personne a {} :dollar:.".format(db[gold_dans_db]), color=white)
			await self.channel.send(embed=embed)
	...
	async def level(self, notation, args) : 
		...
		embed = discord.Embed(title="", description=f"Niveau : `{lvl}`\nxp : `{xp}/{lvl*500}`", color=white)
		await self.channel.send(embed=embed)
	...
	async def give (self, notation, args) : 
		...
		if str(self.author) == cible_user : 
			embed = discord.Embed(title="Vous ne pouvez pas vous donner de l'argent à vous même !!!!".format(prefixe + "give"), description="", color=white)
			await self.channel.send(embed=embed)
			return
		...
		try : 
			...
			embed = discord.Embed(title=f"Moi qui vous croyais moins machiavélique... Vous ne pouvez pas donner un montant négatif !!!", description="", color=white)
			await self.channel.send(embed=embed)
			return

		if not (((gold_of_author - valeur) > 0) or ((gold_of_author - valeur) == 0)) : # Trop pauvre !
			embed = discord.Embed(title=f"Vous ne possédez pas suffisament d'argent pour envoyer {valeur} :dollar: à {cible_user}", description="", color=white)
			await self.channel.send(embed=embed)
			return
			...
			embed = discord.Embed(title="⇄ Transfert effectué : ", description=f"{self.author} : **{gold_of_author}** :dollar: - {valeur} :dollar: = **{gold_of_author - valeur}** :dollar:\n⇄\n{cible_user} : **{gold_of_cible}** :dollar: + {valeur} :dollar: = **{gold_of_cible + valeur}** :dollar:", color=white)
			await self.channel.send(embed=embed)
	...
	async def rank_level (self, notation) : 
		...
		embed = discord.Embed(title="🏆 Classement des niveaux : ", description=f"{to_print}", color=white)
		await self.channel.send(embed=embed)
	...
	async def beg(self) : 
		...
		if int(db[beg_dans_db]) == 0 :
			...
			embed = discord.Embed(title="Tu as mendié un peu d'argent", description=f"*+{rand}* :dollar:", color=white)
			await self.channel.send(embed=embed)
			
		elif int(db[beg_dans_db]) > 0 :
			minute = db[beg_dans_db]
			embed = discord.Embed(title=f"Tu dois encore attendre {minute} min avant de mendier à nouveau.", description="", color=white)
			await self.channel.send(embed=embed)
	...
	async def daily(self) : 
		...
		if int(db[daily_dans_db]) == 0 :
			db[gold_dans_db] = int(db[gold_dans_db]) + int(daily_add + (daily_add * (lvl/20)))
			db[daily_dans_db] = 1439
			embed = discord.Embed(title="Tu as récupéré ta récompense journalière.", description=f"*+{int(daily_add + (daily_add * (lvl/20)))}* :dollar:", color=white)
			await self.channel.send(embed=embed)
	...
	async def ping (self) :
		embed = discord.Embed(title="Pong :ping_pong:", description="", color=white)
		await self.channel.send(embed=embed)
	...
	async def hebdo (self) : 
		...
		if int(db[hebdo_dans_db]) == 0:
			db[gold_dans_db] = int(db[gold_dans_db]) + int(hebdo_add + (hebdo_add * (lvl/20)))
			db[hebdo_dans_db] = 10079
			embed = discord.Embed(title="Tu as récupéré ta récompense hebdomadaire.", description=f"*+{int(hebdo_add + (hebdo_add * (lvl/20)))}* :dollar:", color=white)
			await self.channel.send(embed=embed)
	...
	async def steal(self, notation, args) : 
		...
		if str(self.author) == cible : 
			embed = discord.Embed(title="Vous ne pouvez pas vous voler vous même !!!!", description=" Ou alors, vous êtes plus bête que je ne le pensait.", color=white)
			await self.channel.send(embed=embed)
			return
		...
		if int(db[steal_dans_db]) == 0 :
			...
			try : 
				...
				if shield > 0 : 
					...
					embed = discord.Embed(title=f"Vous vous êtes lamentablement fracassé la tête sur son bouclier :shield:, vous échouez donc à le voler.", description="", color=white)
					await self.channel.send(embed=embed)
					return
			...
			db[gold_dans_db_for_author] = str(gold_of_author + valeur) # Ajoute l'argent à la cible
			db[gold_dans_db_for_cible] = str(gold_of_cible - valeur) # On vole l'argent !
			db[report_dans_db_for_author] = f"5|{cible}"
			db[steal_dans_db] = 1439
			embed = discord.Embed(title=f"Vous avez dérobé dans le porte-feuille bien garni de {cible} *+{valeur}* :dollar: !", description=f"Cet idiot a 5 minutes pour vous rattrapper avant que vous soyez loin ! (avec `{prefixe}report`)\n\n{self.author} : **{gold_of_author}** :dollar: + {valeur} :dollar: = **{gold_of_author + valeur}** :dollar:\n⇄\n{cible} : **{gold_of_cible}** :dollar: - {valeur} :dollar: = **{gold_of_cible - valeur}** :dollar:", color=white)
			...
			heure = minute // 60
			embed = discord.Embed(title=f"Tu dois encore attendre {heure % 24}h {minute % 60}min avant de voler à nouveau.", description="", color=white)
			await self.channel.send(embed=embed)
	...
	async def report(self, notation, args) : 
		...
		if str(self.author) == cible : 
			embed = discord.Embed(title="Vous ne pouvez pas vous dénoncer vous même !!!!", description=" Ou alors, vous êtes plus bête que je ne le pensait.", color=white)
			await self.channel.send(embed=embed)
			return
		...
		try : 
			...
			embed = discord.Embed(title="Cette personne n'est pas un voyou, contrairement à toi, qui a fait appel à la police pour rien !!!!", description="", color=white)
			await self.channel.send(embed=embed)
			return
		
		if value == 0 : 
			embed = discord.Embed(title="Cette personne est déjà loin ! Il aurait fallu m'appeller plus tôt !", description="", color=white)
			await self.channel.send(embed=embed)
			return
		
		if stolen != str(self.author) : 
			embed = discord.Embed(title=f"Seul {stolen} peut m'appeller pour dénoncer un voleur.", description="", color=white)
			await self.channel.send(embed=embed)
			return
		...
		db[gold_dans_db_for_author] = str(gold_of_author + valeur) # On ajoute l'argent
		db[gold_dans_db_for_cible] = str(gold_of_cible - valeur) # On enlève l'argent à la cible
		del db[report_dans_db_for_author]
		embed = discord.Embed(title=f"On a rattrapé ce voleur ! Ce voyou de {cible} vous a payé une amende de *+{valeur}* :dollar: pour sa fourberie !", description=f"Il n'aurait jamais du s'attaquer à vous !\n\n{self.author} : **{gold_of_author}** :dollar: + {valeur} :dollar: = **{gold_of_author + valeur} :dollar:**\n⇄\n{cible} : **{gold_of_cible}** :dollar: - {valeur} :dollar: = **{gold_of_cible - valeur}** :dollar:", color=white)
